...
label scene_20:
    ...
    menu:
        ...
        "Зачем беспокоить человека, может она хочет побыть одна":
            ...
            позавчера вечером"
            grisha "Ну давайте туда скатаемся, выбора особо у нас нет"
            lyonya "А я ведь говорил что что-то случилось! Говорил!!!"
    return
...
# Квартира Аслана
label scene_23:
    play music oxxxymiron fadein 1 fadeout 1 volume 0.2
...
label scene_25:
    ...
    то есть на улице"
    show lyonya_usual_winter at left with dissolve
    ...
    author "Костя ушел в неизвестном направлении"
...
label scene_28:
    ...
    show aslan with dissolve
    aslan "кто я? где я?"
    lyonya "Слыш, ты!!! Что ты с ней сделал???"
    ...
    if lyonya_score >= 2 and valya_score == 1:
        menu:
            "Попросить Лёню вспомнить моменты с Валей":
                ...
                листочки. Я тогда только в чьи-то какашки наступил, а ты мне
                салфетки влажные предложила"
...
label scene_30:
    ...
    if valya_score > 0:
        ...
        grisha "У него интересный предмет, наверное..."
