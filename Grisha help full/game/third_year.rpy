...
# Вводная фраза
label scene_31_a:
    scene bg_spring with fade
    play music neutral_3 fadein 1 fadeout 1 volume 0.5
    author "Третий курс. Ребята уже через многое прошли за время учебы.
    Они пытаются спокойно продолжать учебу, не смотря на все трудности, которые встречаются на их пути."
    return

# Начало 3 курса
label scene_31:
    scene auditorium with fade
    author "Студенты приходят на очередную пару. Внезапно в аудиторию заходит
...
label scene_35:
    ...
    alina "Ангелин, у тебя такая хорошая команда поддержки"
    if valya_score > 0:
        angelina "Да, мне тоже нравится. Это кстати Лёня, Гриша и Валя."
    else:
...
label scene_39:
    ...
    if valya_score > 0:
        ...
        lyonya "Э-э-э...."
        grisha "Ну скажи, что мутите, чего ты"
        ...
        "Пригласить еще Ангелину":
            ...
            menu:
                ...
                "Пригласить на медляк Ангелину":
                    ...
                    grisha "Все, давай без подробностей"
                    if valya_score > 0:
                        author "Лёня с Валей увлеклись разговором между собой"
                        lyonya "Помнишь?"
                        valya "Да-да, такое не забудешь. Круто было!"
                        hide lyonya_usual with dissolve
                        hide valya_usual with dissolve
...
label dance_alina:
    $ alina_score += 1
    author "Плавная музыка заставляет многих людей
    уйти из центра танцпола. Гриша и Алина пошли
...
label dance_angelina:
    ...
    angelina "Ладно, не парься, он не такой как ты подумал. Ты ему кстати понравился,
...
label scene_bar_lyonya:
    ...
    Ну ты понял... Конечно, были еще причины, но эта
    в том числе"
    grisha "А ты откуда знаешь все это, главный
    сплетник Унитеха?"
    lyonya "Э-э-э, ну-у-у..."
    grisha "Ладно, не нравится мне этот тип"
...
label scene_45_angelina:
    ...
    if ang_score >= 8:
        ...
        author "Но Гриша отстраняется от нее."
        angelina "Прости..."
